...
- [x] 全局功能检查
  - [x] 完整链路：登录 → 创建知识库 → 上传 PDF → 等待解析 → 搜索 → 看到结果
  - [x] 用 curl 验证所有 9 个接口
  - [x] 确认 `.gitignore` 排除 `*.db` 和 `uploads/` 下实际文件
- [x] 更新 README（第 4 周完成内容：DB、JWT、解析、FTS5）
- [x] 记录下周计划（Week 5：Qdrant + Embedding + 语义检索 + RAG 问答）

#### 验收标准

- [x] 前端文档列表页支持关键词搜索
- [x] 搜索与状态筛选可同时使用
- [x] 完整链路：登录 → 创建知识库 → 上传 PDF → 解析完成 → 搜索到内容
- [x] README 已更新到第 4 周阶段
