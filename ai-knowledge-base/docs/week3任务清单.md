...
- [x] 用 `ElCard` + `ElStatistic`（或近似组件）替换 Dashboard 手写卡片
- [x] 用 `ElTable` 替换文档列表页手写表格
- [x] 用 `ElTag` 统一替换所有状态标签样式
- [x] 用 `ElButton` 统一替换所有操作按钮
- [x] 检查 6 个页面的间距、字体、颜色是否统一
- [x] 登录页保持简洁，与后台风格协调

### 验收标准

- [x] 工作台页面使用 Element Plus 组件，风格统一
- [x] 文档列表页使用 Element Plus 表格
- [x] 6 个页面视觉风格无明显差异
...
- [x] 为列表页增加 `ElBreadcrumb` 面包屑导航
- [x] 完善 loading 状态（使用 `ElLoading` 或 `v-loading`）
- [x] 完善空状态展示（使用 `ElEmpty`）
- [x] 完善错误状态展示（使用 `ElAlert`）
- [x] 创建知识库成功后跳转到文档列表页
- [x] 上传文档成功后跳转到文档列表页
- [x] 优化页面标题和说明文字
