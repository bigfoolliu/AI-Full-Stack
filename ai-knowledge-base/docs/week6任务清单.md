...
- [x] 前端维护当前会话的消息列表（`ref<Message[]>`）
- [x] 每次 Chat 请求携带 `history: [{role, content}]`
- [x] 后端正确定义 history 在 prompt 中的位置
- [x] 控制上下文长度（超出 token 限制时截断早期消息）
- [x] 实现"新对话"按钮，清空当前上下文
- [x] 实现"重新提问"按钮，重新发送同一条消息
...
- [x] 前端引用来源卡片：文档名 + 片段预览 + 相似度得分
- [x] 引用与回答对应展示（回答段落下方附来源）
- [x] 引用可点击展开查看完整片段
- [x] 空知识库 / 无检索结果时的提示文案
- [x] 消息时间戳展示
- [x] 新消息自动滚屏到页面底部
