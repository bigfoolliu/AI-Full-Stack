...
- [ ] 创建全局 ErrorBoundary 组件（Vue 3 `onErrorCaptured`）
- [ ] 包裹路由页面组件，出错时展示兜底 UI
- [ ] 包装 `axios` 响应拦截器，统一处理 401/403/404/500
- [ ] 401 时自动跳转登录页
- [ ] 页面级别 loading 状态完善（骨架屏或加载指示器）
- [ ] 空状态组件统一（无知识库、无文档、无会话、无搜索结果的占位提示）
- [ ] Toast 提示统一封装（Element Plus ElMessage + ElNotification）
...
- [ ] 安装 `redis-py` 依赖
- [ ] 配置 Redis 连接（`REDIS_URL` 环境变量）
- [ ] 封装 `redis_client.py`（连接池 + 基础 get/set）
- [ ] 搜索缓存：相同 query + top_k 组合缓存 5 分钟
- [ ] 会话列表缓存：知识库会话列表缓存 30 秒
- [ ] 缓存失效策略：文档重新处理后清除该知识库的搜索缓存
- [ ] 添加 `CACHE_ENABLED` 开关，可在页面或环境变量中关闭
...
- [ ] 检查容器化环境下的全链路功能是否正常
- [ ] 补充架构图（前端 → Nginx → Backend → PostgreSQL / Redis / Qdrant → LLM）
- [ ] 更新 README：
  - [ ] 容器化启动说明
  - [ ] 环境变量完整说明
  - [ ] 目录结构说明
...
- [ ] 确保 `make check` 全部通过
- [ ] 审视项目完整度，列出 Week 9 面向项目 2 的计划

### 验收标准

- [ ] 通过 `git clone && docker-compose up` 可完整启动项目
- [ ] 项目 README 和架构图完整
- [ ] 项目达到可向面试官展示的状态
- [ ] 项目 2 的启动计划已就绪
